                    ...
                    switch (parts[0])
                    {
                        ...
                        case "rec":
                            ...
                            cmd.CommandText = String.Format(new System.Globalization.CultureInfo("en-US"),
                                "select top {0} name, result, target, time from ds20_records_turutimo order by ABS(result)", parts[1]);

                            reader = cmd.ExecuteReader();
                            string[] results = new string[int.Parse(parts[1]) + 1];
                            int i = 0;
                            while (reader.Read())
                            {
                                results[i + 1] = ReadSingleRow((IDataRecord)reader);
                                i++;
                            }
                            results[0] = i.ToString();
                            for(int j = 0; j < i + 1; j++)
                            {
                                wtr.WriteLine("rec: {0}", results[j]);
                            }
                            wtr.Flush();
                            reader.Close();
